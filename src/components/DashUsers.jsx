...
import RegisterUser from "./CreateUser"; // Import your RegisterUser component
...
export default function DashUsers() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const res = await apiFetch(`/user/getusers`);
  ...
  const handleDeleteUser = async () => {
    try {
      const res = await apiFetch(
        `/user/delete?userId=${userIdToDelete}`,
        "DELETE"
      );
      const data = await res.json();
    ...
    <div className="table-auto overflow-x-scroll md:mx-auto p-3 scrollbar scrollbar-track-slate-100 scrollbar-thumb-slate-300 dark:scrollbar-track-slate-700 dark:scrollbar-thumb-slate-500">
      ...
      {currentUser.isAdmin && (
        <div className="flex justify-end mb-4">
          <Button gradientDuoTone="purpleToBlue" onClick={openAddUserModal}>
            + Add New User
          </Button>
        </div>
      )}
